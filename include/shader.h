...
public:
    ...
    inline RenderHelp(Bitmap &bitmap)
    {
        _frame_buffer = &bitmap;
        _fb_width = bitmap.GetW();
        _fb_height = bitmap.GetH();
        _depth_buffer = new float *[_fb_height];
        _render_frame = false;
        _render_pixel = true;
        for (int j = 0; j < _fb_height; j++)
        {
            _depth_buffer[j] = new float[_fb_width];
        }
        Clear();
    }
